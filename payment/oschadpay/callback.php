...
require_once(dirname(__FILE__).'/OschadpayView.php');
$fonView = new OschadpayView();
$simpla = new Simpla();
$err = '';
////////////////////////////////////////////////
...
if (empty($_POST))
{
    ...
    list($order_id,) = explode(oschadpaycsl::ORDER_SEPARATOR, $_POST['order_id']);
    ...
    $options = array(
        'merchant' => $settings['oschadpay_merchantid'],
        'secretkey' => $settings['oschadpay_secret']
    );
    $paymentInfo = oschadpaycsl::isPaymentValid($options, $_POST);

    if (!$order->paid) {
        if ($_POST['amount'] / 100 >= round($simpla->money->convert($order->total_price, $payment_method->currency_id, false), 2)) {
            if ($paymentInfo === true) {
                if ($_POST['order_status'] == oschadpaycsl::ORDER_APPROVED) {

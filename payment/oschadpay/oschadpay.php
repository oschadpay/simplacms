...
require_once('payment/oschadpay/oschadpay.cls.php');

class Oschadpay extends Simpla
{	
	public function checkout_form($order_id, $button_text = null)
	{
		...
		// Способ оплаты
		$paymode = $settings['oschadpay_paymode'];

		$success_url = $this->config->root_url.'/order/';
		$result_url = $this->config->root_url.'/payment/oschadpay/callback.php';
		...
		$oplata_args = array(
			'order_id' => $order_id . oschadpaycsl::ORDER_SEPARATOR . time(),
			'merchant_id' => $settings['oschadpay_merchantid'],
			'order_desc' => $desc,
			'amount' => $price * 100,
			'currency' => $currency,
		...
		$oplata_args['signature'] = oschadpaycsl::getSignature($oplata_args, $settings[oschadpay_secret]);
		...
		if ($settings['mode'] == 'redirect'){
			...
			$out = '<script src="https://api.oschadpay.com.ua/static_common/v1/checkout/ipsp.js"></script>
			<div id="checkout">
